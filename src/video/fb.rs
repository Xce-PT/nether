...
impl<'a> Tile<'a>
{
    ...
    pub fn draw_triangle(&mut self, tri: &Triangle, lights: &[Light])
    {
        ...
        let ptx = self.ptx;
        let pty = self.pty;
        let x0 = f32x4::splat(tri.0.proj[0]) - ptx;
        let y0 = f32x4::splat(tri.0.proj[1]) - pty;
        let x1 = f32x4::splat(tri.1.proj[0]) - ptx;
        let y1 = f32x4::splat(tri.1.proj[1]) - pty;
        let x2 = f32x4::splat(tri.2.proj[0]) - ptx;
        let y2 = f32x4::splat(tri.2.proj[1]) - pty;
        ...
        let (tcol, trow, tcolmax, trowmax) =
            if bary0.reduce_min() >= 0.0 && bary1.reduce_min() >= 0.0 && bary2.reduce_min() >= 0.0 {
                ...
                let vx0 = f32x4::splat(tri.0.proj[0]);
                let vy0 = f32x4::splat(tri.0.proj[1]);
                let vx1 = f32x4::splat(tri.1.proj[0]);
                let vy1 = f32x4::splat(tri.1.proj[1]);
                let vx2 = f32x4::splat(tri.2.proj[0]);
                let vy2 = f32x4::splat(tri.2.proj[1]);
                ...
                let x = f32x4::from_array([tri.0.proj[0], tri.1.proj[0], tri.2.proj[0], f32::NAN]);
                let y = f32x4::from_array([tri.0.proj[1], tri.1.proj[1], tri.2.proj[1], f32::NAN]);
                let tminx = f32x4::splat(tmin[0]);
                let tminy = f32x4::splat(tmin[1]);
                let tmaxx = f32x4::splat(tmax[0]);
                let tmaxy = f32x4::splat(tmax[1]);
