...
impl From<[Vector; 4]> for Matrix {
    fn from(vecs: [Vector; 4]) -> Self {
        #[cfg(all(target_arch="aarch64", target_feature="neon"))]
        {
            Self {raw: float32x4x4_t(vecs[0].into_intrinsic(), vecs[1].into_intrinsic(), vecs[2].into_intrinsic(), vecs[3].into_intrinsic())}
        }
        #[cfg(not(all(target_arch="aarch64", target_feature="neon")))]
        {
...
impl Index<usize> for Matrix {
    ...
    fn index(&self, idx: usize) -> &f32 {
        unsafe {
            transmute::<_, &[f32; 16]>(&self.raw).index(idx)
        }
    }
}
...
impl IndexMut<usize> for Matrix {
    ...
    fn index_mut(&mut self, idx: usize) -> &mut f32 {
        unsafe {
            transmute::<_, &mut [f32; 16]>(&mut self.raw).index_mut(idx)
        }
    }
}
